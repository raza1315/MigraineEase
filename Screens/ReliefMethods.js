...
export default function ReliefMethods({ route }) {
  const { intensity, selectedAreas, medsTaken, startTime, endTime } =
    route.params;
  ...
  const handleAddMethod = () => {
    if (newMethod.trim()) {
      const newReliefMethod = {
        id: reliefMethods.length,
        name: newMethod.trim(),
      };
      setReliefMethods([
        reliefMethods[0],
        newReliefMethod,
        ...reliefMethods.slice(1),
      ]);
      setNewMethod("");
      setShowModal(false);
    }
        ...
        {reliefMethods.map((method) => (
          ...
          >
            ...
            <Text
              style={[
                styles.buttonText,
                method.id === "noMeasure" && styles.noMeasureText,
              ]}
            >
              {method.name}
            </Text>
