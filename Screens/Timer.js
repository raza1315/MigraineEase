...
export default function Component() {
  ...
  const formatDate = (date) => {
    ...
    return `${days[date.getDay()]}, ${months[date.getMonth()]} ${date.getDate()}, ${date.getHours()}:${String(date.getMinutes()).padStart(2, '0')}`;
  };
  ...
  const handleEndTimeSelection = (type) => {
    switch (type) {
      case 'still_going':
        ...
        break;
      case 'other':
        setTempDate(startTime ? new Date(Math.max(startTime.getTime(), new Date().getTime())) : new Date());
        setPickerMode('date');
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.timeSection}>
        ...
        <Text style={styles.selectedTime}>{startTime ? formatDate(startTime) : 'Select time'}</Text>
        
        <Text style={styles.presetsLabel}>Time presets</Text>
        <View style={styles.presetButtons}>
          <TouchableOpacity
            style={[styles.presetButton, selectedStartPreset === 'just_now' && styles.selectedPreset]}
          ...
          <TouchableOpacity
            style={[styles.presetButton, selectedStartPreset === '1h_ago' && styles.selectedPreset]}
          ...
          <TouchableOpacity
            style={[styles.presetButton, selectedStartPreset === 'other' && styles.selectedPreset]}
      ...
      <View style={styles.timeSection}>
        ...
        <View style={styles.presetButtons}>
          <TouchableOpacity
            style={[styles.presetButton, endTime === 'Still going' && styles.selectedPreset]}
      ...
      <View style={styles.navigation}>
        ...
        <TouchableOpacity 
          ...
          onPress={() => {console.log(startTime.toISOString(), endTime)
            navigation.navigate('PainArea', {
              startTime: startTime ? startTime.toISOString() : null, // Corrected method
              endTime: typeof endTime !== 'string' && endTime!="Still going" ? endTime.toISOString() : null, // Ensure endTime is handled properly
            });
